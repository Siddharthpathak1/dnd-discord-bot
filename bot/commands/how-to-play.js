  ...
  async execute(interaction) {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '1a) Voice (optional)', value: 'Enable voice automation: run `/listen start <campaign>` to have the bot join voice, transcribe speech, and auto-apply actions (requires `DEEPGRAM_API_KEY` and `AI_API_KEY`). Use `/listen status` to check.', inline: false },
        ...
        { name: '9) Progression & Loot', value: 'Use `/xp <campaign> [amount]` to view or award XP, and `/inventory <campaign>` to see loot. DMs can hand out items with `/loot-add <campaign> <player> <item>`.', inline: false },
        { name: '10) Quests & Scenes', value: 'Manage jobs with `/quest-board <campaign> [quest] [status]`, create NPCs with `/npc-create`, talk to them with `/npc-talk`, and trigger dynamic scenes with `/random-encounter`.', inline: false },
        { name: '11) AI Content', value: 'Use `/ai-campaign` to generate a brand-new campaign, `/ai-trailer` for a cinematic trailer, and `/ai-character` for a level-based character sheet.', inline: false },
        { name: '12) Campaign Tools', value: 'DMs can create a starter game with `/campaign-seed lumenreach-depths`, then use `/session-recap` for a story recap, `/award-title` for achievements, `/help-dnd` for the command reference, and `/how-to-play` for this walkthrough.', inline: false },
        { name: 'Quick Examples', value: '`/roll 2d6+3` — damage\n`/roll 1d20 advantage` — attack with advantage\n`/initiative Lumenreach 14` — add initiative\n`/hp Lumenreach 18` — set HP to 18\n`/map-create Lumen1 https://.../cavern.jpg` — create a map\n`/xp Lumenreach 150` — award XP\n`/listen start Lumenreach` — bot joins voice and listens to auto-resolve events\n`/loot-add Lumenreach @player "Enchanted Dagger"` — give an item to a player', inline: false },
